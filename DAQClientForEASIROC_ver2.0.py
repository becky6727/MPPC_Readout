...
class Application(Tk.Frame):
    ...
    def CreateWidgets(self):
        ...
        self.BlankBtn1 = Tk.Button(self.FrameBtn, text = 'Blank', width = 14)
        self.BlankBtn1.pack(fill = Tk.X, expand = 1, padx = 0, side = Tk.LEFT, anchor = Tk.NW)

        self.BlankBtn2 = Tk.Button(self.FrameBtn, text = 'Blank', width = 14)
        self.BlankBtn2.pack(fill = Tk.X, expand = 1, padx = 0, side = Tk.LEFT, anchor = Tk.NW)
    ...
    def DoDAQ(self):
        ...
        #port no of tcp
        PortTCP = int(self.VarTCP.get())

        #port no of udp
        PortUDP = int(self.VarUDP.get())

        #internal HV setting
        try:
            HV = float(self.VarHV.get())
        except ValueError:
            Msg = 'Internal HV value is invald!'
            self.PopError(Msg)
            raise RuntimeError
        
        if((HV >= 90.0) or (HV <= 0.0)):
            Msg = 'Inernal HV is out of range!!'
            self.PopError(Msg)
            raise RuntimeError
        
        #daq mode selection
        DAQMode = int(self.VarMode.get())
        ...
        #Run number
        RunNumber = int(self.VarRunN.get())

        #number of events to be measured
        try:
            NofEvt = int(self.VarEventN.get())
        except ValueError:
        ...
        #--------- DAQ main ---------#
        if(self.PopStart()):
            #initialize and take data...
            self.AppDAQ(IP, PortTCP, PortUDP, HV, DAQMode, RunNumber, NofEvt)
        else:
    ...
    def QuitDAQ(self):
        ...
        while True:
            try:
                ...
                self.UDP.Finalize()
                break
            except sitcpy.rbcp.RbcpTimeout as e:
                print 'UDP connection timeout is detected! try reconncet...'
                #reconnect
                time.sleep(1)
                self.UDP = SetUDPCtrl.SetUDPCtrl(IPAddr, PortUDP)
                pass
            pass
    ...
    def AppDAQ(self, IPAddr, PortTCP, PortUDP, HV, DAQMode, RunNo, NofEvt):
        ...
        #connection
        Sock = self.DAQSockTCP.ConnectTCP(IPAddr, PortTCP)
        ...
        InitializeFPGA = SetFPGA.SetFPGA(Sock, ConfigDict)
        ...
        InitializeSlowCtrl = SetSlowControl.SetSlowControl(Sock, ConfigDict)
        InitializeAnalog = SetAnalogOutput.SetAnalogOutput(Sock, ConfigDict)
...
root.title('DAQ software for EASIROC module')
